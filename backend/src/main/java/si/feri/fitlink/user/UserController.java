...
public class UserController {
    ...
    /** Registers an Expo push token (used by the Expo mobile-app client). */
    @PostMapping("/me/push-token")
    public ResponseEntity<Void> updatePushToken(
        @AuthenticationPrincipal AuthPrincipal principal,
        @Valid @RequestBody PushTokenRequest body
    ) {
        userService.setExpoPushToken(principal.uid(), body.token());
        return ResponseEntity.noContent().build();
    }
    ...
    public record PushTokenRequest(@NotBlank @Size(max = 4096) String token) {}
}
